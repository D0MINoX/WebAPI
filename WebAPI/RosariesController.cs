...
namespace WebAPI
{
        ...
        [HttpGet("available-rosaries/{parishId}")]
        public async Task<IActionResult> GetAvailableRosaries(int parishId)
        {
          
            var result = await _context.Rosary
                .Where(r=> r.ParishValue==parishId)
                .Select(r => new
                {
                    Id = r.Id,
                    Name = r.Name
